...
public OnPluginStart()
{
	...
	// This plugin version
	g_hPluginVersion = CreateConVar("sm_mixmod_version", PLUGIN_VERSION, "Mix Plugin version.", FCVAR_SS_ADDED|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
{
	if (hasMixStarted && IsClientInGame(client))
	{
		PrintToChat(client, "\x04[%s]:x\03 比赛开始！ \x01请勿抢位 \x01玩得开心，过得愉快", MODNAME);
		PrintToChat(client, "\x04[%s]:x\03 比赛开始！ \x01请勿中途退出 \x01玩得开心，过得愉快", MODNAME);
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/
		...
		if (IsClientInGame(i))
		{
			...
			PrintToChat(i, "\x04[%s]:\x03 你的武器，金钱，得分已被刷新！", MODNAME);
		}
		...
		if (hasMixStarted)
		{
			...
			if (g_CurrentHalf == 1)
			{
				...
				if (GetConVarInt(g_CvarShowScores) == 1)
					PrintToChatAll("\x04[%s]:\x03 回合\x03 %d \x04- 半场进度\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScoreH1, teamBName, g_nTScoreH1);
...
public Action:InformMix(Handle:timer, any:team)
{
	...
	if (team == 3)
		PrintToChatAll("\x04[%s]: \x03赢家是:\x04 %s", MODNAME, teamAName);
	else if (team == 2)
		PrintToChatAll("\x04[%s]: \x03赢家是:\x04 %s", MODNAME, teamBName);
	else if (team == 1)
...
public Event_PlayerDisconnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((GetConVarInt(g_CvarAutoMixEnabled) == 1)) // Auto-Mix is running...
	{
		new client = GetClientOfUserId(GetEventInt(event, "userid"));
		if (client == 0) {
			return;
...
public Action:Command_JoinTeam(client, args)
{
	...
	if ((hasMixStarted == true) && (GetClientTeam(client) == CS_TEAM_CT || GetClientTeam(client) == CS_TEAM_T)) {
		...
		if (team == CS_TEAM_CT && GetTeamClientCount(CS_TEAM_CT) >= 5 && GetClientTeam(client) == CS_TEAM_T) {
			PrintToChat(client, "\x04[%s]:\x03 你无法更改队伍！", MODNAME);
			return Plugin_Handled;
		}
		if (team == CS_TEAM_T && GetTeamClientCount(CS_TEAM_T) >= 5 && GetClientTeam(client) == CS_TEAM_CT) {
			PrintToChat(client, "\x04[%s]:\x03 你无法更改队伍！", MODNAME);
			return Plugin_Handled;
		}
	}
...
public Action:Command_Ready(client, args)
{
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (!g_AllowReady)
		{
			PrintToChat(client, "\x04[%s]:\x03 已经有十名玩家准备！", MODNAME);
			return Plugin_Handled;
		}
		...
		if (hasMixStarted)
		{
			...
			{
				PrintToChat(client, "\x04[%s]:\x03 满十启动了", MODNAME);
				return Plugin_Handled;
			}
		}
		...
		{
			PrintToChat(client, "\x04[%s]:\x03 你必须选择一个队伍才能使用该指令。", MODNAME);
			return Plugin_Handled;
		}
		...
		PrintToChat(client, "\x04[%s]:\x03 你已经准备好咯！", MODNAME);
...
{
	g_HasVoteMap = true;
	CreateTimer(2.0, VoteMapMenuShow, _, TIMER_FLAG_NO_MAPCHANGE);

}

Action VoteMapMenuShow(Handle timer) {
...
public Action:Command_ShowTeams(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (g_IsItManual || !hasMixStarted)
		{
			...
			{
				if (client != 0)
					PrintToChat(client, "\x04[%s]:\x03 这个指令仅在满十启动时候才可用");
		...
		if (client == 0)
			PrintToChatAll("\x04[%s]:\x03 队伍:", MODNAME);
		else
			PrintToChat(client, "\x04[%s]:\x03 队伍:", MODNAME);
...
public Action:Command_UnReady(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (GetConVarInt(g_CvarAutoMixEnabled) == 0) // Auto-Mix is not running...
		{
			PrintToChat(client, "\x04[%s]:\x03 满十自动启动已经开始 当前指令被禁用.", MODNAME);
			return Plugin_Handled;
		}
		
		if (hasMixStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 Mix插件已经开启 当前指令被禁用", MODNAME);
			return Plugin_Handled;
		}
...
public Action:Command_ShowMvp(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!hasMixStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 满十配置未加载 当前指令被禁用", MODNAME);
			return Plugin_Handled;
		}
		
		if (GetConVarInt(g_CvarShowMVP) == 0)
		{
			PrintToChat(client, "\x04[%s]:\x03 MVP数据加载禁用 当前指令被禁用", MODNAME);
			return Plugin_Handled;
		}
		...
		if (kills <= 0)
		{
			PrintToChat(client, "\x04[%s]:\x03 MVP还未被选呢 等一会", MODNAME);
			return Plugin_Handled;
		}
		PrintToChat(client, "\x04[%s]:\x03 MVP:", MODNAME);
		PrintToChat(client, "------------------");
		PrintToChat(client, "\x03 - The \x01MVP:\x04 %s , 击杀了:\x04 %d 敌人！", mvpName, kills);
		...
		PrintToChat(client, "\x03 你整死了 \x04 %d \x03 个敌人", g_ScoresOfTheGame[client]);
...
Action UpdateReadyPanel()
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && (IsClientSourceTV(i) != true) && (IsClientReplay(i) != true))
		{
			if (GetClientTeam(i) == CS_TEAM_SPECTATOR) {

				GetClientName(i, name, sizeof(name));
				Format(Spec, sizeof(Spec), "%s %s\n\n", Spec, name);
			}
		}
	}
...
public Action:SwapTimer(Handle:timer)
{
	...
	if (GetConVarInt(g_CvarHalfAutoLiveStart) == 1) // Admin need to write: !live to start...
		didLiveStarted = true;
}
...
{
	PrintToChatAll("\x04[%s]:\x03 地图将要更改为 %s", MODNAME, mapname);
...
TryStartMatch()
{
    ...
    {
		PrintToChatAll("\x04[%s]:\x03 即将更换地图", MODNAME);
